<?php
/**
 * admin_release_escrow.php
 * - Admin-only action: release escrow to seller
 * - Sets escrow_transactions.status = 'released'
 * - Sets orders.status = 'completed'
 * - Notifies seller via messages
...
$note      = trim($_POST['note'] ?? 'Released via admin console');
...
try {
  if (isset($pdo) && $pdo instanceof PDO) {
    ...
    $q = $pdo->prepare("
      SELECT e.order_id, e.status AS e_status,
             o.status AS o_status, o.seller_id
    ...
    // Mutations
    $pdo->prepare("UPDATE escrow_transactions SET status='released' WHERE id=?")->execute([$escrow_id]);
    $pdo->prepare("UPDATE orders SET status='completed' WHERE id=?")->execute([(int)$row['order_id']]);

    // Notify seller
    $msg = $pdo->prepare("INSERT INTO messages (sender_id, receiver_id, message, is_read) VALUES (?,?,?,0)");
    $msg->execute([$_SESSION['user_id'], (int)$row['seller_id'], "Admin released escrow for order #{$row['order_id']}"]);
    ...
    $u1 = $conn->prepare("UPDATE escrow_transactions SET status='released' WHERE id=?");
    $u1->bind_param("i", $escrow_id); $u1->execute();

    $u2 = $conn->prepare("UPDATE orders SET status='completed' WHERE id=?");
    ...
    $txt= "Admin released escrow for order #{$row['order_id']}";
    $m->bind_param("iis", $_SESSION['user_id'], $row['seller_id'], $txt); $m->execute();
  ...
  error_log("admin_release_escrow error: ".$e->getMessage());
  http_response_code(500); exit('Release failed');
}
